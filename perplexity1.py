...
def main(argv):
    ...
    for opt, arg in opts:
        if opt in ('-h', '--help'):
            ...
            global auth, auth_login, auth_pass
            auth = True
...
def request(index):
    ...
    while not ex.is_set():
        ...
        try:
            ...
            pass
        if err_count >= 20:
            print("Proxy " + ips[index] + " has been kicked from attack due to its nonoperability")
            return

def cloudFlareCheck():
    global url
    if isCloudFlare(url) is True:
        print("*** Your target is hiding behind CloudFlare! This attack may not entail any consequences to the victim's web-site.")
        time.sleep(1)
        for i in range(5, 0, -1):
            # PERBAIKAN

            print('Your attack will be launched in ' + str(i) + ' seconds...')
            time.sleep(1)
...
def startAttack():
    ...
    try:
        ...
        print('Give up to ' + str(timeout) + ' seconds to release the threads...')
        for t in threads:
            t.join()
...
def showUsage():
    ...
    print("Please, read more about arguments in GitHub repository!")
